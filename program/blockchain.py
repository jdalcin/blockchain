...
class Blockchain:
    ...
    def isBlockchainValid(self, blockchain):
        ...
        blockIndex = 1
        while blockIndex < len(blockchain):
            ...
            encryptedGuess = hashlib.sha256(encodedGuess).hexdigest()
...
@app.route('/is-valid-blockchain', methods = ['GET'])
def isValidBlockchain():
    isValid = blockchain.isBlockchainValid(blockchain.blockchain)
    response = {
        'message': isValid
    }
    return jsonify(response), 200
